...
public abstract class ForceLocalizeSystem<TMod, TSelf> where TSelf : ForceLocalizeSystem<TMod, TSelf> /* where T : Mod */
{
    ...
    public static void Localize(MethodInfo? methodInfo, Dictionary<string, string> localizations)
    {
        ...
        new ILHook(methodInfo, il =>
        {
            ...
            //TryGotoNext 单向(向下找) 找到后面给的条件后返回
            //ilOpit => 是个委托
            //如果ilOpit是MatchLdstr 那么就true了
            while (cursor.TryGotoNext(ilOpit => ilOpit.MatchLdstr(out str)))
            {
                if (string.IsNullOrWhiteSpace(str) && cursor.Next != null && localizations.TryGetValue(str, out string value))
                {
