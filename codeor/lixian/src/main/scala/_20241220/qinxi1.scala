...
object qinxi1 {
    def main(args: Array[String]): Unit = {
        ...
        val mtime = sp.sql("select max(etl_date) from ods.customer_inf").first()(0)
        val newdate = sp.sql(s"select * from ods.customer_inf where etl_date = '${mtime}'")

        val otime = sp.sql("select max(etl_date) from dwd.dim_customer_inf").first()(0)
        val olddate = sp.sql(s"select * from dwd.dim_customer_inf where etl_date = '${otime}'")
        ...
        val win1 = Window.partitionBy("customer_id").orderBy("modified_time")
