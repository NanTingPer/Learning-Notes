...
object xi3_dim_province {
    def main(args: Array[String]): Unit = {
        ...
        //ods
        val odstable = spark.sql("select * from ods.base_province").where(col("etl_date") === "20250406")

        //dwd
        val dwdtable = spark.sql("select * from dwd.dim_province")
...
//select id, sku_desc, dwd_insert_user, dwd_modify_time, etl_date from dwd.dim_sku_info where etl_date = '20250406' and id >= 15 and id <= 20 order by id;
