...
object CQ6 {
    def main(args: Array[String]): Unit = {
        ...
        val sql_table_name = "order_detail"
        val hudi_table_name = "order_detail"
        ...
        val hudi_max_time = hudi.select(max("create_time")).first()(0)

        //updata
        sql_table
            .where(col("create_time") > hudi_max_time)
