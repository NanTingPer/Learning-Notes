...
namespace ExampleMod.Content.Projectiles
{
	...
	//ExampleCustomSwingSword 是一个使用手持射弹进行自定义摆动的剑的示例
	//如果您想制作具有复杂挥舞行为的近战武器，这非常有用
	//请注意，此弹丸仅涵盖 2 个相对简单的挥舞，其他一切都取决于您
	//除了自定义动画之外，Colliding 中的自定义碰撞代码对于此武器也非常重要
	public class ExampleCustomSwingProjectile : ModProjectile
	{
		...
		/// <summary>
		/// 摆动程度(目标阈值,相对于swingRange)
		/// </summary>
		private const float FIRSTHALFSWING = 0.45f;                 // How much of the swing happens before it reaches the target angle (in relation to v)
		...
		/// <summary>
		/// 每个阶段的持续时间(哪个阶段就是哪个阶段的时间)
		/// </summary>
		private ref float Timer => ref Projectile.ai[2]; // Timer to keep track of progression of each stage

		/// <summary>
		/// 相对初始角度的偏移(当前弹幕相对于初始角的偏移)
		/// </summary>
		...
		public override void OnSpawn(IEntitySource source) {
			...
			Projectile.spriteDirection = Main.MouseWorld.X > Owner.MountedCenter.X ? 1 : -1;//通过鼠标与玩家的X轴进行比较，判断该往哪边挥舞
			float targetAngle = (Main.MouseWorld - Owner.MountedCenter).ToRotation();//计算鼠标与玩家的角度
		...
		public override bool PreDraw(ref Color lightColor) {
			...
			if (Projectile.spriteDirection > 0) {
				...
				effects = SpriteEffects.FlipHorizontally;//水平旋转
			}
