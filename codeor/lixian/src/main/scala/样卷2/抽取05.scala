...
//        val huditable = spark.read.format("hudi").load("hdfs:///user/hive/warehouse/ods_ds_hudi.db/order_info")
...
//        val maxtime = huditable.where(col("etl_date") === maxetl).select(max(greatest(col("operate_time"), col("create_time")))).first()(0)
        ...
        //增量数据
        val zlsj = spark.read.jdbc("jdbc:mysql://192.168.45.13:3306/shtd_store?useSSL=false", "order_detail", conf)
