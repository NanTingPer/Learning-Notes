...
object qinxi3 {
    def main(args: Array[String]): Unit = {
        ...
        val mtime = sp.sql("select max(etl_date) from ods.product_info").first()(0)
        val newdate = sp.sql(s"select * from ods.product_info where etl_date = '${mtime}'")

        val otime = sp.sql("select max(etl_date) from dwd.dim_product_info").first()(0)
        val olddate = sp.sql(s"select * from dwd.dim_product_info where etl_date = '${otime}'")
        ...
        val win1 = Window.partitionBy("product_core").orderBy("modified_time")
