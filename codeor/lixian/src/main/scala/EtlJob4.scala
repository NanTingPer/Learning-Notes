...
object EtlJob4 {
  def main(args: Array[String]): Unit = {
    ...
    spark.table("ods.coupon_use")
    spark.sql("select greatest(max(get_time),max(used_time),max(pay_time)) from ods.coupon_use where get_time != 'NULL' and used_time !='NULL' and pay_time != 'NULL'").show()
  }
}