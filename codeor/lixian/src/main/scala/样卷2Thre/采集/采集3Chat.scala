...
object 采集3Chat {
  def main(args: Array[String]): Unit = {
      ...
      val MysqlTableName:String = "base_province"
    val HudiTableName:String = "base_province"
    ...
    val max_increase_time: Long = sparkSession.sql(
      s"""
        |from ods_ds_hudi.${HudiTableName} select max(id) as max_increase_time
        |""".stripMargin).collect()(0).getLong(0)
    // 获取ods层表列的顺序
    val columns: Array[String] = sparkSession.sql(s"from ods_ds_hudi.${HudiTableName}  select *  where etl_date='19971201' limit 1")
    ...
    // 读取mysql数据
   val jdbcUrl:String =  s"jdbc:mysql://192.168.45.13:3306/shtd_store?useSSL=false"
    val properties: Properties = new Properties()
    properties.put("user", "root") //用户名
    properties.put("password","123456") // 密码
    properties.put("driver", "com.mysql.jdbc.Driver") // 驱动名称
      val dataFrame = sparkSession.read.jdbc(jdbcUrl, MysqlTableName, properties)
          .where(col("id") > 0)
          .withColumn("etl_date", lit(yd)) //TODO 添加分区字段以及值
          .withColumn("create_time", date_format(current_timestamp(), "yyyy-MM-dd HH:mm:ss").cast("timestamp"))
          .select(columns.map(col): _*)
      dataFrame // 按顺序获取数据 防止顺序错位
      .write.format("hudi")
      .options(getQuickstartWriteConfigs)
      .option(TBL_NAME.key(),"base_province") //todo 设置hudi 表名
      .option(DataSourceWriteOptions.RECORDKEY_FIELD.key(),"id") //todo 设置primaryKey
      .option(DataSourceWriteOptions.PRECOMBINE_FIELD.key(),"create_time") //todo 设置preCombineField
    ...
    // 查看表分区数据
    sparkSession.sql("show partitions ods_ds_hudi.base_province;").show(false)
    // 关闭SparkSession
    sparkSession.stop()
  }
