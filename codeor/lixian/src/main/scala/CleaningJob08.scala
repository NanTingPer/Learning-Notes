...
//  }
  ...
  def readHBase(spark:SparkSession) = {
    ...
    hbaseConf.set("hbase.zookeeper.quorum", "192.168.45.10") //设置zooKeeper集群地址，也可以通过将hbase-site.xml拷贝到resources目录下
    hbaseConf.set("hbase.zookeeper.property.clientPort", "2181") //设置zookeeper连接端口，默认2181
    hbaseConf.set("hbase.master", "192.168.45.10:16000")     // 设置HBase HMaster
    // 设置从HBase哪张表读取数据
    val tablename = "ods:product_browse" // 要读取的hbase表名
    hbaseConf.set(TableInputFormat.INPUT_TABLE, "ods:product_browse")  // 注意：读取时使用TableInputFormat
