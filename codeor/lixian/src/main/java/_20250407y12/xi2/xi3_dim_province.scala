...
object xi3_dim_province {
    def main(args: Array[String]): Unit = {
        ...
        //ods
        val odstable = spark.read.load("hdfs:///user/hive/warehouse/ods_ds_hudi.db/base_province").where(col("etl_date") === "20250406")
          .drop("_hoodie_commit_time", "_hoodie_commit_seqno", "_hoodie_record_key", "_hoodie_partition_path", "_hoodie_file_name")
        //dwd
        val dwdtable = spark.read.load("hdfs:///user/hive/warehouse/dwd_ds_hudi.db/dim_province")
