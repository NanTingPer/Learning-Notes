    ...
    static async Task ExecMethod(List<string> ItemName, List<string> filyList, string fileType)
    {
        ...
        while (ItemName.Count != 0) {
            for(int i = 0; i < ItemName.Count; i ++) {
                ...
                #region 文本识别
                //注意这里 ，目标主机的文本识别需要放到这个路径
                var r =Environment.CurrentDirectory;
                var tEngine = new TesseractEngine(Path.Combine(Environment.CurrentDirectory, "tessdata"), "chi_sim", EngineMode.Default);
                Pix pix = Pix.LoadFromFile(filePath);
                TesseractPage page = tEngine.Process(pix);
                string str = page.GetText();
                str = Regex.Replace(str, @"\s", "");

                if (str.Contains(tk2) || str.Contains(tk3)) {
                    AwaitItem.Add(pageName);
