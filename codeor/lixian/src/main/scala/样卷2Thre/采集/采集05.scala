...
object 采集05 {
    def main(args: Array[String]): Unit = {
        ...
        val sqldata = spark.read.jdbc("jdbc:mysql://192.168.45.13:3306/shtd_store?useSSL=false", "order_info", conf)
        val hudidata = spark.read.format("hudi").load("hdfs:///user/hive/warehouse/ods_ds_hudi.db/order_info")
        ...
        val maxid = hudidata.select(max(greatest(col("operate_time"), col("create_time")))).first()(0)
        val update = sqldata.where(greatest(col("operate_time"), col("create_time")) > maxid)
        ...
        update
            ...
            .save("hdfs:///user/hive/warehouse/ods_ds_hudi.db/order_info")
    }
}
