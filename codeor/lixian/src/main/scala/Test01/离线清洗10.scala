...
object 离线清洗10 {
    def main(args: Array[String]): Unit = {
        ...
        val mt = sq.sql("select max(etl_date) from ods.customer_level_inf").first()(0)
        sq.sql(s"select * from ods.customer_level_inf where etl_date = '${mt}'")
                ...
                .saveAsTable("dwd.dim_customer_level_inf")
    }
